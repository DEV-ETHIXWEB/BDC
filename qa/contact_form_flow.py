...
with sync_playwright() as p:
    ...
    f = pg.locator('.cb-form'); f.scroll_into_view_if_needed()
    for t in ('Kids and families', 'What to bring'): f.locator(f"button:has-text('{t}')").click()
    OK('C8 several topics can be picked at once', f.locator("button[aria-pressed=true]:has-text('Kids and families'), button[aria-checked=true]:has-text('Kids and families')").count() == 1 and f.locator("[data-selected]").count() == 2)
    f.locator("button:has-text('Half Day Trip')").first.click(); f.locator("button:has-text('Half Day Drift Boat')").first.click()
    OK('C9 trip is single choice', f.locator("[data-selected]:has-text('Half Day')").count() == 1 and f.locator("[data-selected]:has-text('Half Day Drift Boat')").count() == 1)
    f.locator("button:has-text('Other')").first.click()
    OK('C10 Other asks for details', 'Tell us what you need' in f.inner_text())
    f.locator('input[name=name]').fill('Test User'); f.locator('input[name=email]').fill('t@example.com')
    f.locator('button[type=submit]').click(); pg.wait_for_timeout(300)
    OK('C11 Other without details is blocked', 'You chose Other' in f.inner_text())
    f.locator("button:has-text('Call')").first.click(); f.locator('textarea[name=message]').fill('A birthday trip for six')
    f.locator('button[type=submit]').click(); pg.wait_for_timeout(300)
    OK('C12 choosing Call needs a phone number', 'Add a phone number' in f.inner_text())
    f.locator("button:has-text('Email')").first.click(); f.locator('button[type=submit]').click(); pg.wait_for_timeout(700)
    href = pg.locator("a:has-text('Open email again')").get_attribute('href') or ''
    OK('C13 mailto lists the picked topics, trip and message', 'Kids%20and%20families' in href and 'What%20to%20bring' in href and 'Half%20Day%20Drift%20Boat' in href and 'birthday' in href)
    pg.reload(wait_until='load'); pg.wait_for_timeout(1000)

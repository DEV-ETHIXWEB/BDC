...
with sync_playwright() as p:
    ...
    # validation
    pg.locator('.cb-form').scroll_into_view_if_needed(); pg.wait_for_timeout(1000)  # client:visible hydrates on scroll
    ...
    f = pg.locator('.cb-form'); f.scroll_into_view_if_needed(); pg.wait_for_timeout(1000)
    ...
    f.locator('textarea[name=message]').fill('A birthday trip for six'); f.locator('button[type=submit]').click(); pg.wait_for_timeout(700)
    href = pg.locator("a:has-text('Open email again')").get_attribute('href') or ''
    OK('C13 mailto lists the picked topics and message', 'Kids%20and%20families' in href and 'What%20to%20bring' in href and 'birthday' in href)
    pg.reload(wait_until='load'); pg.wait_for_timeout(1000)
